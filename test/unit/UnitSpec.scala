...
class UnitSpec extends PlaySpec {
	...
	"SearchForm" must {
		"apply successfully from request" in {
			...
			val call = controllers.routes.SearchController.search()
			implicit val request: Request[_] = FakeRequest(call)
				.withFormUrlEncodedBody(
					"name" -> "11206"
				)

			// A successful binding using an implicit request will give you 
			// a form with a value.
			val boundForm = controller.searchForm.bindFromRequest()
			// You can then get the widget data out and test it.
			val searchEntry = boundForm.value.get

			searchEntry.zipcode must equal("11206")
		}

		"apply successfully from map" in {
			// You can also bind directy from a map, if you don't have a request handy.
			val data = Map("zipcode" -> "11206")
			// A successful binding will give you a form with a value.
			val boundForm = controller.searchForm.bind(data)
			// You can then get the widget data out and test it.
			val searchEntry = boundForm.value.get
		...
		"show errors when applied unsuccessfully" in {
			...
			val data = Map("zipcode" -> "112061")

			// ...and binding the form will show errors.
			val errorForm = controller.searchForm.bind(data)
			// You can then get the widget data out and test it.
			...
			formError.key must equal("zipcode")
			...
			// The message is in the language that was "preferred" by the 
			// request's Messages component.  The closest messages file i.e. 
			// messages.en is looked up, and then the constraint key 
			// ("error.min") is looked up.  If there is no user defined
			// mapping, then the default messages file is "messages.default", 
			// which has
			//
			// error.min=Must be greater or equal to {0}
			//
			// but in this case, we haven't passed in a request, because we 
			// called bind instead of bindFromRequest!
			//
			// As such, when there is nothing in scope, the error message is 
			// the key itself.
			formError.message must equal("error.min")

			// You get the content of the message by calling 
			// Messages(key, args) with an in scope MessagesProvider.
			//
			// Usually you'll do this through dependency injection with
			// app.inject[MessagesApi] and messageApi.preferred(request), but 
			// we can do it by hand here just to demonstrate what happens 
			// underneath the hood:
			...
			val messagesProvider: MessagesProvider = messagesApi.preferred(
				Seq(lang)
			)
